...
type OrderRow = { id: string; order_code: string; status: string; total: number; balance_due: number; order_type: string; opened_at: string; table_id: string | null; waiter_id: string };

export default async function OrdersPage() {
  const context = await requireRole("admin", "waiter", "cashier");
  const admin = createAdminClient();
  const { data: orderData, error } = await admin.from("orders").select("id, order_code, status, total, balance_due, order_type, opened_at, table_id, waiter_id").eq("branch_id", context.profile.branchId).not("status", "eq", "cancelled").order("created_at", { ascending: false }).limit(100);

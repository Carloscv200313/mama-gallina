...
import { formatLimaDateTime, getLimaDayRange } from "@/lib/pos/date";
...
export default async function SalesPage() {
  ...
  const { data: orderData } = await admin.from("orders").select("id, order_code, total, payment_total, order_type, paid_at, waiter_id").eq("branch_id", context.profile.branchId).eq("status", "paid").gte("paid_at", limaDayStart.toISOString()).lt("paid_at", limaDayEnd.toISOString()).order("paid_at", { ascending: false }).limit(200);

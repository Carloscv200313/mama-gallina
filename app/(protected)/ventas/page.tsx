...
export default async function SalesPage() {
  ...
  const { data: orderData } = await admin.from("orders").select("id, order_code, total, payment_total, order_type, paid_at, waiter_id").eq("branch_id", context.profile.branchId).eq("status", "paid").gte("paid_at", today.toISOString()).order("paid_at", { ascending: false }).limit(200);
  ...
  return <div className="mx-auto max-w-7xl space-y-6"><div><p className="text-sm font-semibold uppercase tracking-[0.16em] text-brand-olive">Historial financiero</p><h1 className="mt-2 font-display text-4xl font-bold">Ventas</h1><p className="mt-2 text-brand-olive">Ventas pagadas del día y distribución por método.</p></div><section className="grid gap-4 sm:grid-cols-3"><Metric title="Ventas del día" value={formatCurrency(total)} icon={TrendingUp} /><Metric title="Pedidos cerrados" value={String(orders.length)} icon={ReceiptText} /><Metric title="Ticket promedio" value={formatCurrency(average)} icon={BarChart3} /></section><div className="grid gap-5 lg:grid-cols-[280px_1fr]"><Card><CardHeader><CardTitle>Por método</CardTitle></CardHeader><CardContent className="space-y-3">{Object.entries(byMethod).length ? Object.entries(byMethod).map(([method, amount]) => <div key={method} className="flex justify-between text-sm"><span>{method === "cash" ? "Efectivo" : method === "plin" ? "Plin" : "Transferencia"}</span><span className="font-semibold">{formatCurrency(amount)}</span></div>) : <p className="text-sm text-brand-olive">Sin pagos registrados.</p>}</CardContent></Card><Card><CardHeader><CardTitle>Ventas cerradas hoy</CardTitle></CardHeader><CardContent className="space-y-3">{orders.length === 0 ? <p className="py-8 text-center text-sm text-brand-olive">No hay ventas cerradas hoy.</p> : orders.map((order) => <div key={order.id} className="flex flex-col gap-2 rounded-xl border border-brand-olive/10 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold">{order.order_code}</p><p className="text-xs text-brand-olive">{order.order_type === "takeaway" ? "Para llevar" : "Mesa"} · {staffMap.get(order.waiter_id) ?? "Sin asignar"} · {new Date(order.paid_at).toLocaleTimeString("es-PE", { hour: "2-digit", minute: "2-digit" })}</p></div><span className="font-display font-bold">{formatCurrency(Number(order.total))}</span></div>)}</CardContent></Card></div></div>;
}

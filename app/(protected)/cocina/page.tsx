...
export default async function KitchenPage() {
  ...
  const { data: orderData } = await admin.from("orders").select("id, order_code, table_id, opened_at, status").eq("branch_id", context.profile.branchId).in("status", ["sent_to_kitchen", "preparing", "partially_ready", "ready"]).order("opened_at");
  const orders = (orderData ?? []) as Array<{ id: string; order_code: string; table_id: string | null; opened_at: string; status: string }>;

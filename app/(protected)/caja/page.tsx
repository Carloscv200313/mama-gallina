...
type PendingPayment = { id: string; orderId: string; orderCode: string; amount: number; method: string; status: string; operationNumber: string | null; secureUrl: string | null; rejectionReason: string | null };
...
export default async function CashPage({ searchParams }: { searchParams: Promise<{ orderId?: string }> }) {
  ...
  const [{ data: sessionData }, { data: orderData }, { data: paymentData }] = await Promise.all([
    ...
    admin.from("orders").select("id, order_code, total, payment_total, balance_due, status").eq("branch_id", context.profile.branchId).in("status", ["payment_pending", "delivered", "ready"]).order("created_at", { ascending: false }).limit(50),
    admin.from("payments").select("id, order_id, amount, method, status, operation_number, rejection_reason").eq("branch_id", context.profile.branchId).in("status", ["pending_evidence", "pending_verification", "rejected"]).order("created_at", { ascending: false }).limit(50),
  ]);
  ...
  const payments = (paymentData ?? []) as Array<{ id: string; order_id: string; amount: number; method: string; status: string; operation_number: string | null; rejection_reason: string | null }>;
  ...
  const pendingPayments: PendingPayment[] = payments.map((payment) => ({ id: payment.id, orderId: payment.order_id, orderCode: orderMap.get(payment.order_id)?.order_code ?? "Pedido", amount: Number(payment.amount), method: payment.method, status: payment.status, operationNumber: payment.operation_number, secureUrl: evidenceMap.get(payment.id) ?? null, rejectionReason: payment.rejection_reason }));

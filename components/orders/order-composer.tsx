...
import { createAdditionalOrder, createOrder, updateOrder } from "@/app/actions/orders";
...
type Props = {
  catalog: { categories: CatalogCategory[]; products: CatalogProduct[] };
  table: { id: string; tableNumber: number; name: string | null } | null;
  initialOrderType: "dine_in" | "takeaway";
  editingOrderId?: string;
  additionalOrderId?: string;
  initialCart?: OrderComposerItem[];
  initialNotes?: string;
...
function mergeCartItem(current: OrderComposerItem[], next: OrderComposerItem[]) {
  ...
  for (const item of next) {
    const matchingIndex = result.findIndex((value) => cartItemFingerprint(value) === cartItemFingerprint(item));
    if (matchingIndex < 0) result.push(item);
    else result[matchingIndex] = { ...result[matchingIndex], quantity: result[matchingIndex].quantity + item.quantity, modifierIds: mergeModifierIds(result[matchingIndex], item), plateSelections: [...result[matchingIndex].plateSelections, ...item.plateSelections] };
  }
  return result;
}
...
export function OrderComposer({ catalog, table, initialOrderType, editingOrderId, additionalOrderId, initialCart, initialNotes = "", editReasonRequired = false }: Props) {
  ...
  function toggleOption(group: CatalogModifierGroup, optionId: string) {
    setSelectedOptions((current) => {
      ...
      if (group.selectionMode === "single") return { ...current, [group.id]: values.includes(optionId) ? [] : [optionId] };
  ...
  function toggleUnitOption(plateIndex: number, group: CatalogModifierGroup, optionId: string) {
    setUnitSelections((current) => {
      ...
      const nextValues = group.selectionMode === "single" ? (values.includes(optionId) ? [] : [optionId]) : values.includes(optionId) ? values.filter((value) => value !== optionId) : [...values, optionId];
  ...
  function removePlate(plateIndex: number) {
    setQuantity((current) => Math.max(1, current - 1));
    setUnitSelections((current) => {
      const remaining = Object.entries(current).filter(([index]) => Number(index) !== plateIndex);
      return Object.fromEntries(remaining.map(([, plate], index) => [index, plate]));
    });
  }
  ...
  function submitOrder() {
    ...
    startTransition(async () => {
      ...
      const result = additionalOrderId ? await createAdditionalOrder({ parentOrderId: additionalOrderId, notes, idempotencyKey: crypto.randomUUID(), items }) : editingOrderId ? await updateOrder({ orderId: editingOrderId, notes, reason: editReason, items }) : await createOrder({ orderType, tableId: orderType === "dine_in" ? table?.id ?? null : null, peopleCount: orderType === "dine_in" ? 2 : null, customerName: "", notes, idempotencyKey: crypto.randomUUID(), items });
  ...
  return <div className="mx-auto max-w-7xl space-y-5 pb-24">
    <div className="flex items-center gap-3">
      <Button asChild variant="ghost" size="icon"><Link href={additionalOrderId || editingOrderId ? `/pedidos/${additionalOrderId ?? editingOrderId}` : "/mesas"}><ArrowLeft className="size-5" /></Link></Button>
      <div><p className="text-sm font-semibold uppercase tracking-[0.16em] text-brand-olive">{additionalOrderId ? "Nueva tanda" : editingOrderId ? "Editar pedido" : "Nuevo pedido"}</p><h1 className="font-display text-3xl font-bold">{table ? `Mesa ${table.tableNumber}` : additionalOrderId ? "Agregar pedido" : editingOrderId ? "Pedido solicitado" : "Pedido para llevar"}</h1></div>
      <Badge className="ml-auto">{formatCurrency(total)}</Badge>
    </div>
    <div className="grid gap-5 xl:grid-cols-[1fr_380px]">
      <div className="space-y-5">
        <Card><CardContent className="space-y-4 pt-5">
          <div className="relative"><Search className="absolute left-3 top-3.5 size-4 text-brand-olive" /><Input className="pl-10" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar en la carta..." /></div>
          <div className="flex gap-2 overflow-x-auto pb-1"><Button type="button" size="sm" variant={categoryId === "all" ? "default" : "outline"} onClick={() => setCategoryId("all")}>Todo</Button>{catalog.categories.map((category) => <Button type="button" size="sm" variant={categoryId === category.id ? "default" : "outline"} key={category.id} onClick={() => setCategoryId(category.id)}>{category.name}</Button>)}</div>
          <div className="grid gap-3 sm:grid-cols-2">{products.map((product) => <button type="button" key={product.id} onClick={() => openProduct(product)} className="rounded-2xl border border-brand-olive/10 p-4 text-left transition hover:border-brand-olive/40 hover:bg-brand-cream"><div className="flex items-start justify-between gap-2"><div><p className="font-semibold">{product.name}</p><p className="mt-1 line-clamp-2 text-xs text-brand-olive">{product.description ?? ""}</p></div><Plus className="size-5 shrink-0 text-brand-olive" /></div><p className="mt-4 font-display text-lg font-bold">{product.variants.length ? `Desde ${formatCurrency(Math.min(...product.variants.map((variant) => variant.salePrice)))}` : formatCurrency(product.salePrice)}</p></button>)}</div>
          {products.length === 0 ? <p className="py-8 text-center text-sm text-brand-olive">No se encontraron productos.</p> : null}
        </CardContent></Card>
      </div>
      <Card className="h-fit xl:sticky xl:top-24"><CardHeader><CardTitle className="flex items-center justify-between"><span className="flex items-center gap-2"><ShoppingBasket className="size-5" /> Resumen</span><Badge variant="muted">{cart.length} líneas</Badge></CardTitle></CardHeader><CardContent className="space-y-3">
        {cart.map((item) => <div key={item.localId} className="rounded-xl border border-brand-olive/10 p-3"><div className="flex items-start justify-between gap-2"><div><p className="font-semibold">{item.quantity} × {item.productName}</p>{item.variantName ? <p className="text-xs text-brand-olive">{item.variantName}</p> : null}{item.modifierLabels.length ? <p className="text-xs text-brand-olive">{item.modifierLabels.join(" · ")}</p> : null}</div><div className="flex items-center gap-2"><button type="button" className="text-brand-olive" onClick={() => editCartItem(item)} aria-label="Editar producto"><Pencil className="size-4" /></button><button type="button" className="text-red-600" onClick={() => setCart((current) => current.filter((value) => value.localId !== item.localId))} aria-label="Quitar producto"><Trash2 className="size-4" /></button></div></div><div className="mt-2 flex items-center justify-between text-sm"><span>{formatCurrency((item.unitPrice + item.modifiersTotal) * item.quantity)}</span>{item.hasPerPlateOptions ? <span className="text-xs text-brand-olive">{item.quantity} platos configurados</span> : <div className="flex items-center gap-2"><button type="button" onClick={() => setCart((current) => current.map((value) => value.localId === item.localId ? { ...value, quantity: Math.max(1, value.quantity - 1) } : value))}><Minus className="size-4" /></button><span>{item.quantity}</span><button type="button" onClick={() => setCart((current) => current.map((value) => value.localId === item.localId ? { ...value, quantity: Math.min(99, value.quantity + 1) } : value))}><Plus className="size-4" /></button></div>}</div></div>)}
        {cart.length === 0 ? <div className="rounded-xl bg-brand-cream p-6 text-center text-sm text-brand-olive">Selecciona productos para comenzar.</div> : null}
        <textarea className="min-h-20 w-full rounded-xl border border-brand-olive/20 p-3 text-sm outline-none focus:border-brand-olive" value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Observación general" />
        {editingOrderId && editReasonRequired ? <textarea className="min-h-20 w-full rounded-xl border border-amber-300 bg-amber-50 p-3 text-sm outline-none focus:border-amber-500" value={editReason} onChange={(event) => setEditReason(event.target.value)} placeholder="Motivo de la edición (obligatorio)" /> : null}
        {error ? <p className="rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</p> : null}
        <Button type="button" className="w-full" size="lg" disabled={isPending || !cart.length || Boolean(editingOrderId && editReasonRequired && editReason.trim().length < 3)} onClick={submitOrder}>{isPending ? "Enviando..." : additionalOrderId ? "Enviar nueva tanda a cocina" : editingOrderId ? "Guardar cambios" : "Enviar pedido a cocina"}</Button>
      </CardContent></Card>
    </div>
    {selected ? <div className="fixed inset-0 z-50 grid place-items-center bg-brand-forest/40 p-4"><div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl sm:p-6">
      <div className="flex items-start justify-between gap-4"><div><p className="font-display text-2xl font-bold">{selected.name}</p><p className="text-sm text-brand-olive">{editingLocalId ? "Edita la configuración de esta línea." : "Personaliza el producto"}</p></div><div className="flex shrink-0 items-center gap-3">{getPerPlateGroups(selected).length ? <button type="button" onClick={addPlate} disabled={quantity >= 99} className="rounded-lg px-2 py-1 text-sm font-semibold text-brand-forest transition hover:bg-brand-cream disabled:opacity-40">+ Agregar plato</button> : null}<button type="button" onClick={() => { setSelected(null); setEditingLocalId(null); }} className="text-brand-olive">Cerrar</button></div></div>
      <div className={`mt-5 grid gap-3 ${selected.variants.length ? "sm:grid-cols-[minmax(0,1fr)_140px]" : "sm:flex sm:justify-end"}`}>
        {selected.variants.length ? <label className="block text-sm font-semibold">Variante<select className="mt-2 min-h-12 w-full rounded-xl border border-brand-olive/20 bg-white px-3" value={variantId ?? ""} onChange={(event) => setVariantId(event.target.value)}>{selected.variants.map((variant) => <option key={variant.id} value={variant.id}>{variant.name} · {formatCurrency(variant.salePrice)}</option>)}</select></label> : null}
      </div>
      {getRegularGroups(selected).map((group) => <fieldset key={group.id} className="mt-5"><legend className="font-semibold">{group.name} {group.isRequired ? <span className="text-red-600">*</span> : null}</legend>{group.description ? <p className="mt-1 text-xs text-brand-olive">{group.description}</p> : null}<div className="mt-2 space-y-2">{group.options.map((option) => <label key={option.id} className="flex items-center justify-between rounded-xl border border-brand-olive/10 p-3 text-sm"><span className="flex items-center gap-2"><input type={group.selectionMode === "single" ? "radio" : "checkbox"} name={group.id} checked={(selectedOptions[group.id] ?? []).includes(option.id)} onChange={() => toggleOption(group, option.id)} />{option.name}</span>{option.additionalPrice > 0 ? <span>+ {formatCurrency(option.additionalPrice)}</span> : null}</label>)}</div></fieldset>)}
      {getPerPlateGroups(selected).length ? <fieldset className="mt-5"><legend className="font-semibold">Personalización por plato</legend><p className="mt-1 text-xs text-brand-olive">Cada tarjeta representa un plato. Puedes elegir sabores, presa y preferencias de forma independiente.</p><div className="mt-3 space-y-3">{Array.from({ length: quantity }, (_, plateIndex) => <div key={plateIndex} className="rounded-xl border border-brand-olive/10 bg-brand-cream/40 p-3"><div className="flex items-center justify-between gap-3"><p className="text-sm font-semibold">Plato {plateIndex + 1}</p><button type="button" onClick={() => removePlate(plateIndex)} disabled={quantity <= 1} className="inline-flex items-center gap-1 rounded-lg px-2 py-1 text-xs font-semibold text-red-600 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-40"><Trash2 className="size-3.5" /> Quitar plato</button></div><div className="mt-3 grid gap-3 sm:grid-cols-2">{getPerPlateGroups(selected).map((group) => { const variant = selected.variants.find((item) => item.id === variantId); const isFlavorGroup = group.name.trim().toLocaleLowerCase().includes("sabores"); const maxSelections = isFlavorGroup && variant?.maxFlavors ? variant.maxFlavors : group.maxSelections; return <div key={group.id} className="text-sm font-semibold">{groupDisplayName(group)}{group.isRequired ? <span className="text-red-600"> *</span> : null}<span className="mt-2 block"><MultiCheckboxSelect options={getProductGroupOptions(selected, group)} selectedIds={unitSelections[plateIndex]?.[group.id] ?? []} onChange={(optionId) => toggleUnitOption(plateIndex, group, optionId)} placeholder={groupPlaceholder(group)} maxSelections={maxSelections} /></span></div>; })}</div></div>)}</div></fieldset> : null}
      <label className="mt-5 block text-sm font-semibold">Nota<textarea className="mt-2 min-h-20 w-full rounded-xl border border-brand-olive/20 p-3 text-sm" value={itemNotes} onChange={(event) => setItemNotes(event.target.value)} placeholder="Observación para cocina (opcional)" /></label>
      {error ? <p className="mt-3 text-sm text-red-700">{error}</p> : null}
      <Button type="button" className="mt-5 w-full" onClick={addSelected}>{editingLocalId ? <Pencil className="size-4" /> : <Plus className="size-4" />} {editingLocalId ? "Guardar cambios" : "Agregar al pedido"}</Button>
    </div></div> : null}
  </div>;
}

...
import { CashSessionSalesButton } from "@/components/cash/cash-session-sales";
...
function CashSessionHistory({ rows }: { rows: CashSessionHistory[] }) {
  return <Card><CardHeader><CardTitle className="flex items-center gap-2"><History className="size-5" /> Historial de cierres</CardTitle></CardHeader><CardContent className="space-y-3">{rows.length === 0 ? <p className="py-6 text-center text-sm text-brand-olive">Todavía no hay sesiones de caja registradas.</p> : rows.map((row) => <div key={row.id} className="rounded-xl border border-brand-olive/10 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-semibold">{formatCashDate(row.openedAt)} · {row.cashierName}</p><p className="mt-1 text-xs text-brand-olive">Apertura {formatCurrency(row.openingAmount)} · {row.closedAt ? `Cierre ${formatCashDate(row.closedAt)}` : "Sesión activa"}</p></div><div className="flex items-center gap-2"><Badge variant={row.status === "open" ? "success" : row.status === "closed_with_difference" ? "danger" : "muted"}>{row.status === "open" ? "Abierta" : row.status === "closed_with_difference" ? "Cerrada con diferencia" : "Cerrada"}</Badge><CashSessionSalesButton sessionId={row.id} /></div></div>{row.status !== "open" ? <div className="mt-3 grid gap-2 text-sm sm:grid-cols-3"><div><p className="text-xs text-brand-olive">Esperado</p><p className="font-semibold">{formatCurrency(row.expectedAmount ?? 0)}</p></div><div><p className="text-xs text-brand-olive">Contado</p><p className="font-semibold">{formatCurrency(row.countedAmount ?? 0)}</p></div><div><p className="text-xs text-brand-olive">Diferencia</p><p className={`font-semibold ${(row.difference ?? 0) === 0 ? "text-emerald-700" : "text-red-700"}`}>{formatSignedCurrency(row.difference ?? 0)}</p></div></div> : null}</div>)}</CardContent></Card>;
}
